...
### Code Quality (impl-1)
- [ ] Review canvasStore.ts (37KB) for any code that could be extracted into utility functions
- [ ] Add JSDoc comments to all exported functions in src/stores/*.ts
- [ ] Ensure consistent error handling patterns across all tool files in src/tools/
- [ ] Review and improve TypeScript types - eliminate any `any` types
- [ ] Extract magic numbers into named constants (especially in rendering code)
...
- [ ] Verify paste operations work when clipboard is empty or contains invalid data
...
- [ ] Profile the canvas render loop - identify any unnecessary redraws
- [ ] Review mouse event handlers for expensive operations that should be throttled
- [ ] Check for memory leaks in event listener cleanup (component unmounts)
- [ ] Verify quadtree.ts is being used effectively for spatial queries
- [ ] Ensure large object counts (100+) don't degrade selection/drag performance
