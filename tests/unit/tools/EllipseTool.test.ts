...
describe('EllipseTool', () => {
  ...
  beforeEach(() => {
    ...
    tool = new EllipseTool(mockContext);
  });
  ...
  describe('onMouseMove', () => {
    ...
    it('handles negative drag direction (top-left)', () => {
      ...
      // Move to create ellipse going up-left
    ...
    it('constrains to circle with shift key', () => {
      // Start drawing
      const downEvent = createMockMouseEvent({ canvasX: 50, canvasY: 50 });
      tool.onMouseDown(downEvent);

      // Move with shift to create circle
      ...
      const updateCall = vi.mocked(mockContext.updateObject).mock.calls[0];
      // Should be a circle - the larger dimension wins
  ...
  describe('onMouseUp', () => {
    it('finalizes ellipse creation and switches to select tool', () => {
      // Start drawing
      const downEvent = createMockMouseEvent({ canvasX: 50, canvasY: 50 });
      tool.onMouseDown(downEvent);

      // Create a valid size ellipse
    ...
    it('deletes ellipse if too small', () => {
      // Start drawing
      const downEvent = createMockMouseEvent({ canvasX: 50, canvasY: 50 });
      tool.onMouseDown(downEvent);

      // Create a tiny ellipse (accidental click)
